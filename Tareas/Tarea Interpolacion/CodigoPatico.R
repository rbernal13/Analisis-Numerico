...
plot(x,y, pch=20, cex=1, col = "red", asp=1,xlab="X", ylab="Y", main="Lucas")

vx1 = c(x[1:21])
vy1 = c(y[1:21])
splines = splinefun(vx1, vy1, method = "fmm")
curve(splines(x), add = TRUE, col="black", from= vx1[1], to=vx1[length(vx1)] )

vx2 = c(x[21:23])
vy2 = c(y[21:23])
splines = splinefun(vx2, vy2, method = "fmm")
curve(splines(x), add = TRUE, col="blue", from= vx2[1], to=vx2[length(vx2)] )
...
curve(splines(x), add = TRUE, col="blue", from= vx4[1], to=vx4[length(vx4)] )

vx5 = c(x[28:31])
vy5 = c(y[28:31])
splines = splinefun(vx5, vy5, method = "fmm")
curve(splines(x), add = TRUE, col="blue", from= vx5[1], to=vx5[length(vx5)] )
...
curve(splines(x), add = TRUE, col="blue", from= vx7[1], to=vx7[length(vx7)] )

vx8 = c(x[36:37])
vy8 = c(y[36:37])
splines = splinefun(vx8, vy8, method = "fmm")
curve(splines(x), add = TRUE, col="blue", from= vx8[1], to=vx8[length(vx8)] )
